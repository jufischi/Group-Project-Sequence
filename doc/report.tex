...
Starting with the parents of the leaves, the forward pass fills for every node a
list detailing what choosing a certain label would incur in
cost. For this, the cost for the transition from each possible state of the
...
The Sankoff algorithm was implemented using the Python \todo{version number}
...
airpots and pycountry \cite{theunePycountryISOCountry} to translate letter codes
of countries.

To compare our implementation in terms of runtime with the implementation that
was provided in \cite{reimeringPhylogeographicReconstructionUsing2020}, we
...
distance matrix than than the original implementation.
